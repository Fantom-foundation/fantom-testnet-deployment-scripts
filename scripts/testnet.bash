...
if [ ! -d "$DATAL_DIR/dag1_data_dir" ]; then
    echo "$DATAL_DIR/dag1_data_dir is not found; can not run"
    exit 2
fi

GOMAXPROCS=$(($logicalCpuCount - 1)) "$BUILD_DIR/lachesis_$TARGET_OS" run --datadir "$DATAL_DIR/dag1_data_dir/$node" --store --listen="$node_addr":12000 --log=warn --heartbeat=5s -p "$node_addr":9000 -s "$node_addr":9090 --syslog
